...
//补充
Matrix *Hilbert(int order,Matrix *output){/*
 ...
 * 可以调用如, help("help")，查看help函数的使用方法和内容*/
    OS_Err_Collector(">>HELP(");
    OS_Err_Collector(file_name);
    OS_Err_Collector(")\n");
    ...
    if (fp == NULL) {
        OS_Err_Collector(temp_route);
        OS_Err_Collector(" can not open!\n");
    } else {
        fscanf(fp, "%c", &ch);
        while (!feof(fp)) {
            putchar(ch);
...
//补充
Matrix *Matrix_gen(int row, int column, MATRIX_TYPE *data,Matrix* Ouput) {/*
...
//补充
Matrix *Matrix_copy(Matrix *_mat_sourse,Matrix* Output) {/*
 * Copy to a new Matrix (create).
 * 复制矩阵（生成新矩阵）*/
    #ifndef DISABLE_MALLOC
        Matrix *_mat_copy = Matrix_gen(_mat_sourse->row, _mat_sourse->column, _mat_sourse->data);
    #else
        Output=Matrix_gen(_mat_sourse->row, _mat_sourse->column,output_data,Output);
        memcpy(Output->data,_mat_sourse->data,sizeof(_mat_sourse->data));
        Matrix *_mat_copy = Ouput;
    #endif
    return _mat_copy;
}
 ...
 * Norm (1/ 2/ p/ INF/ FRO).
 * 向量/矩阵范数 Setting in [1/ 2/ p/ INF/ FRO]
 * 1: L1范数
 * 2：L2范数
 * p: p范数
 * INF: 无穷范数
 ...
 * Reference: [幂法-算法参考](https://max.book118.com/html/2017/0527/109650252.shtm) */
 ...
 * _mat_result = Max_eigenvalue(_mat)
 * Reference:
 * 1. [幂法-算法参考](https://max.book118.com/html/2017/0527/109650252.shtm)
 * 2. [求解-特征向量参考](https://blog.csdn.net/w_weixiaotao/article/details/111868086) */
    Matrix ** M_array_eigen_vec = NULL;
    if (_mat->column == _mat->row) {
        M_array_eigen_vec = (Matrix **)OS_Malloc(sizeof(Matrix *)*2); // 保存Q/R矩阵地址
        enum{val=0, vec=1};
        ...
        for(eigen_count=0;eigen_count<dim;eigen_count++){
            ...
            for (i = 0; i < dim-1; i++){
                // Note: 感谢 645770225, 具体参考[issue-10](https://github.com/Amoiensis/Matrix_hub/issues/10)
 ...
 * Householder变换
 * Referemce:
 * 1. [householder变换](https://blog.csdn.net/qq_40922398/article/details/112788453)
 */
    Matrix *H = NULL;

//    // 此处感谢 645770225 提初的修改建议, 具体见 [issue-11](https://github.com/Amoiensis/Matrix_hub/issues/11).
 ...
 * */
    ...
    if (_mat->column == _mat->row) {
        ...
        for(i=1;i<dim;i++){
            // 此处"_mat"修改为Ri; 感谢645770225, issue: https://github.com/Amoiensis/Matrix_hub/issues/8;
        ...
        OS_Err_Collector(M_householder_027); // 待完善补充
        OS_Commander("pause");
    }
    return h_Mat;
}
 ...
 * 矩阵QR分解.
 * */
    Matrix ** M_array_Q_R = (Matrix **)OS_Malloc(sizeof(Matrix *)*2); // 保存Q/R矩阵地址
    enum{q=0, r=1};
    M_array_Q_R[q] = NULL;
    M_array_Q_R[r] = NULL;
...
//        x = M_Cut(_mat,i+1,_END_,i+1,i+1);
    ...
    D = M_I(dim);
    for(i=0;i<dim;i++){
        // 此处"D->data"进行修改; 感谢 wtyhainan, issue: https://github.com/Amoiensis/Matrix_hub/issues/9;
 ...
 * 求矩阵的 左/右逆、伪逆, 支持方法: 1. 直接求逆([_INV_L_, _INV_R_]); 2. SVD 分解([_SVD_])
 * */
